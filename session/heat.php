...
if (!empty($_POST)) {
	...
	$query = 'SELECT s.`ID`,s.`NAME`, s.`LOGIN`, s.`AGE`,g.`NAME` AS `GROUP` FROM `students` AS s JOIN `groops` AS g ON g.ID = s.ID_GROUP;';
...
<br>
<form action="heat.php" method="POST">
    <input type="text" name="name" placeholder="Имя"><br>
    <input type="text" name="login" placeholder="Логин"><br>
    <input type="text" name="age" placeholder="Возраст"><br>
    <select name="group">
    <?php 
    $query = 'SELECT `ID`, `NAME` FROM `groops`';
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
    while ($row = mysqli_fetch_assoc($result)) {
        echo '<option>'.$rov['NAME'].'</option>';
    }
    ?>
    </select><br>
    <input type="submit">
